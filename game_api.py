...
class ConnectFourBoard :
    ...
    def same_board_array(first, other):
        ...
        try:
            first.board_array and other.board_array
        except:
            return False
        return first.board_array == other.board_array
...
def indexed_path(state, move_indexes=[]) :
    return reduce(lambda states, index : states + [states[-1].generate_next_states()[index]],
                  move_indexes, [state])
move_sequence = indexed_path

#  f = lambda xs, i : [xs[i]]
#   return reduce(lambda states, index : states[-1].generate_next_moves(), move_indexes[state])
